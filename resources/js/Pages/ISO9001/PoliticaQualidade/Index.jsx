...
export default function Index({ auth, politica, companies, users, currentCompanyId }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        revisor_id: politica.revisor_id || '',
        aprovador_id: politica.aprovador_id || '',
    ...
    const isRascunho = politica.status === 'rascunho' || politica.status === 'devolvida';
    const isAguardandoRevisao = politica.status === 'aguardando_revisao';
    const isAguardandoAprovacao = politica.status === 'aguardando_aprovacao';
    const isAprovada = politica.status === 'aprovada';
                    ...
                    {companies && companies.length > 0 && (
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    window.location.href = route('politica-qualidade.index', { company_id: e.target.value });
                                }}
                            >
                                {companies.map(c => (
                ...
                <div className="max-w-[95%] mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg p-6 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {isAprovada && (
                                <a 
                                    href={route('politica-qualidade.pdf', { company_id: currentCompanyId })} 
                            ...
                            {canManage && !isAprovada && (
                                <div className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                                    ...
                                    <div className="space-y-3">
                                        {isRascunho && (
                                            <button
                                                onClick={() => handleAction(route('politica-qualidade.enviar-revisao'))}
                                                disabled={processing}
                                        ...
                                        {isAguardandoRevisao && (
                                            <>
                                                <button
                                                    onClick={() => handleAction(route('politica-qualidade.aprovar-revisao'))}
                                                    disabled={processing || politica.elaborador_id === auth.user.id}
                                                    title={politica.elaborador_id === auth.user.id ? "Você não pode revisar um documento que elaborou." : ""}
                                                ...
                                                <button
                                                    onClick={() => handleAction(route('politica-qualidade.devolver'))}
                                                    disabled={processing}
                                        ...
                                        {isAguardandoAprovacao && (
                                            <>
                                                <button
                                                    onClick={() => handleAction(route('politica-qualidade.aprovar-final'))}
                                                    disabled={processing}
                                    ...
                                    {isAguardandoRevisao && politica.elaborador_id === auth.user.id && (
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-600 dark:text-slate-400">{politica.elaborador ? politica.elaborador.name : 'Pendente'}</p>
                                            {politica.data_elaboracao && (
                                                <p className="text-xs text-slate-400 mt-1 flex items-center">
                                                    <Clock className="h-3 w-3 mr-1" /> {dayjs(politica.data_elaboracao).format('DD/MM/YYYY HH:mm')}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {/* Revisor */}
                                    <div className="flex gap-4">
                                        <div className={`mt-1 p-2 rounded-full h-min ${politica.data_revisao ? 'bg-blue-50 dark:bg-blue-900/30' : 'bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700'}`}>
                                            <CheckSquare className={`h-4 w-4 ${politica.data_revisao ? 'text-blue-500' : 'text-slate-400'}`} />
                                        </div>
                                        <div>
                                            <p className="text-sm font-semibold text-slate-900 dark:text-white">Revisão {politica.revisor && !politica.data_revisao ? '(Designado)' : ''}</p>
                                            <p className="text-sm text-slate-600 dark:text-slate-400">{politica.revisor ? politica.revisor.name : 'Pendente'}</p>
                                            {politica.data_revisao && (
                                                <p className="text-xs text-slate-400 mt-1 flex items-center">
                                                    <Clock className="h-3 w-3 mr-1" /> {dayjs(politica.data_revisao).format('DD/MM/YYYY HH:mm')}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {/* Aprovador */}
                                    <div className="flex gap-4">
                                        <div className={`mt-1 p-2 rounded-full h-min ${politica.data_aprovacao ? 'bg-emerald-50 dark:bg-emerald-900/30' : 'bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700'}`}>
                                            <CheckCircle className={`h-4 w-4 ${politica.data_aprovacao ? 'text-emerald-500' : 'text-slate-400'}`} />
                                        </div>
                                        <div>
                                            <p className="text-sm font-semibold text-slate-900 dark:text-white">Aprovação {politica.aprovador && !politica.data_aprovacao ? '(Designado)' : ''}</p>
                                            <p className="text-sm text-slate-600 dark:text-slate-400">{politica.aprovador ? politica.aprovador.name : 'Pendente'}</p>
                                            {politica.data_aprovacao && (
                                                <p className="text-xs text-slate-400 mt-1 flex items-center">
                                                    <Clock className="h-3 w-3 mr-1" /> {dayjs(politica.data_aprovacao).format('DD/MM/YYYY HH:mm')}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {politica.hash_assinatura && (
                                        <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-400 break-all font-mono bg-slate-50 dark:bg-slate-900 p-2 rounded">{politica.hash_assinatura}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
