...
import { Plus, Edit, Trash2, Eye, Search, AlertTriangle, CheckCircle, Clock, XCircle } from 'lucide-react';
import { format } from 'date-fns';
import { ptBR } from 'date-fns/locale';

export default function Index({ auth, mapas, filters, companies, currentCompanyId }) {
    const isMasterAdmin = auth.user.is_master_admin;
    const [search, setSearch] = useState(filters.search || '');

    const handleDelete = (id) => {
        if (confirm('Tem certeza que deseja excluir este mapa de risco de forma permanente?')) {
            router.delete(route('mapas-risco.destroy', id));
        }
    };

    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('mapas-risco.index'), { search, company_id: currentCompanyId });
    };
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Mapas de Riscos</h2>}
        >
            <Head title="Mapas de Riscos" />
                    ...
                    {isMasterAdmin && (
                        <div className="mb-6 bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
                            ...
                            <select
                                ...
                                onChange={(e) => router.get(route('mapas-risco.index'), { company_id: e.target.value, search })}
                            >
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                                ...
                                <div className="flex gap-2 w-full md:w-auto">
                                    <Link
                                        href={route('mapas-risco.create', { company_id: currentCompanyId })}
                                ...
                                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                                    <thead className="bg-slate-50 dark:bg-slate-900/40">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Setor</th>
                                            <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Data Mapeamento</th>
                                    ...
                                    <tbody className="bg-white divide-y divide-slate-200 dark:bg-slate-800 dark:divide-slate-700">
                                        {mapas.data.length > 0 ? (
                                            mapas.data.map(mapa => (
                                                <tr key={mapa.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors">
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm font-semibold text-slate-900 dark:text-white">{mapa.titulo}</div>
                                                        {isMasterAdmin && (
                                                            <div className="text-xs text-slate-400 mt-0.5">{mapa.company?.nome_fantasia || 'Sem empresa'}</div>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-300">
                                                        {mapa.data_mapeamento ? format(new Date(mapa.data_mapeamento), 'dd/MM/yyyy', { locale: ptBR }) : '-'}
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-300">
                                                        {mapa.aprovador?.name || 'Não atribuído'}
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-center">
                                                        {getStatusBadge(mapa.status)}
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <div className="flex justify-end gap-2">
                                                            <Link
                                                                href={route('mapas-risco.show', mapa.id)}
                                                            ...
                                                            {(mapa.status === 'draft' || mapa.status === 'rejected') && (
                                                                <Link
                                                                    href={route('mapas-risco.edit', mapa.id)}
                                            ...
                                            <tr>
                                                <td colSpan="6" className="px-6 py-10 text-center text-sm text-slate-500 dark:text-slate-400">
                                                    Nenhum mapa de risco cadastrado.
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
                            ...
                            {/* Paginação */}
                            {mapas.links && mapas.links.length > 3 && (
                                <div className="mt-6 flex justify-center gap-1">
                                    {mapas.links.map((link, i) => (
                                        <button
                                            key={i}
                                            disabled={!link.url}
