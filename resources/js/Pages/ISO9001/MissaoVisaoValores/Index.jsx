...
export default function Index({ auth, mvv, companies, users, currentCompanyId }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        conteudo: mvv.conteudo || '',
        revisor_id: mvv.revisor_id || '',
        aprovador_id: mvv.aprovador_id || '',
    ...
    const isRascunho = mvv.status === 'rascunho' || mvv.status === 'devolvida';
    const isAguardandoRevisao = mvv.status === 'aguardando_revisao';
    const isAguardandoAprovacao = mvv.status === 'aguardando_aprovacao';
    const isAprovada = mvv.status === 'aprovada';
    ...
    const handleDelete = () => {
        if (confirm('Tem certeza que deseja excluir o conteúdo? Ele voltará para o status de Rascunho.')) {
            setData('conteudo', '');
            post(route('missao-visao-valores.salvar-rascunho'), { preserveScroll: true });
        }
    };
                    ...
                    {companies && companies.length > 0 && (
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    window.location.href = route('missao-visao-valores.index', { company_id: e.target.value });
                                }}
                            >
                                {companies.map(c => (
                ...
                <div className="w-full sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg p-6 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-4">
                            {getStatusBadge(mvv.status)}
                            {isAprovada && (
                                <a 
                                    href={route('missao-visao-valores.pdf', { company_id: currentCompanyId })} 
                            ...
                            {isEditing ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-end pt-8 gap-2">
                                        ...
                                        <button
                                            onClick={() => handleAction(route('missao-visao-valores.salvar-rascunho'))}
                                            disabled={processing}
                            ...
                            {canManage && !isAprovada && (
                                <div className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                                    ...
                                    <div className="space-y-3">
                                        {isRascunho && (
                                            <button
                                                onClick={() => handleAction(route('missao-visao-valores.enviar-revisao'))}
                                                disabled={processing}
                                        ...
                                        {isAguardandoRevisao && (
                                            <>
                                                <button
                                                    onClick={() => handleAction(route('missao-visao-valores.aprovar-revisao'))}
                                                    disabled={processing || mvv.elaborador_id === auth.user.id}
                                                    title={mvv.elaborador_id === auth.user.id ? "Você não pode revisar um documento que elaborou." : ""}
                                                ...
                                                <button
                                                    onClick={() => handleAction(route('missao-visao-valores.devolver'))}
                                                    disabled={processing}
                                        ...
                                        {isAguardandoAprovacao && (
                                            <>
                                                <button
                                                    onClick={() => handleAction(route('missao-visao-valores.aprovar-final'))}
                                                    disabled={processing}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-600 dark:text-slate-400">{mvv.elaborador ? mvv.elaborador.name : 'Pendente'}</p>
                                            {mvv.data_elaboracao && (
                                                <p className="text-xs text-slate-400 mt-1 flex items-center">
                                                    <Clock className="h-3 w-3 mr-1" /> {dayjs(mvv.data_elaboracao).format('DD/MM/YYYY HH:mm')}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {/* Revisor */}
                                    <div className="flex gap-4">
                                        <div className={`mt-1 p-2 rounded-full h-min ${mvv.data_revisao ? 'bg-blue-50 dark:bg-blue-900/30' : 'bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700'}`}>
                                            <CheckSquare className={`h-4 w-4 ${mvv.data_revisao ? 'text-blue-500' : 'text-slate-400'}`} />
                                        </div>
                                        <div>
                                            <p className="text-sm font-semibold text-slate-900 dark:text-white">Revisão {mvv.revisor && !mvv.data_revisao ? '(Designado)' : ''}</p>
                                            <p className="text-sm text-slate-600 dark:text-slate-400">{mvv.revisor ? mvv.revisor.name : 'Pendente'}</p>
                                            {mvv.data_revisao && (
                                                <p className="text-xs text-slate-400 mt-1 flex items-center">
                                                    <Clock className="h-3 w-3 mr-1" /> {dayjs(mvv.data_revisao).format('DD/MM/YYYY HH:mm')}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {/* Aprovador */}
                                    <div className="flex gap-4">
                                        <div className={`mt-1 p-2 rounded-full h-min ${mvv.data_aprovacao ? 'bg-emerald-50 dark:bg-emerald-900/30' : 'bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700'}`}>
                                            <CheckCircle className={`h-4 w-4 ${mvv.data_aprovacao ? 'text-emerald-500' : 'text-slate-400'}`} />
                                        </div>
                                        <div>
                                            <p className="text-sm font-semibold text-slate-900 dark:text-white">Aprovação {mvv.aprovador && !mvv.data_aprovacao ? '(Designado)' : ''}</p>
                                            <p className="text-sm text-slate-600 dark:text-slate-400">{mvv.aprovador ? mvv.aprovador.name : 'Pendente'}</p>
                                            {mvv.data_aprovacao && (
                                                <p className="text-xs text-slate-400 mt-1 flex items-center">
                                                    <Clock className="h-3 w-3 mr-1" /> {dayjs(mvv.data_aprovacao).format('DD/MM/YYYY HH:mm')}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    {mvv.hash_assinatura && (
                                        <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-400 break-all font-mono bg-slate-50 dark:bg-slate-900 p-2 rounded">{mvv.hash_assinatura}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
